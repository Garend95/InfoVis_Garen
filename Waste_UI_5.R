...
categ <- c("Waste received from other organizations",
...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    sidebarMenu(
      ...
      pickerInput(
        inputId = "categExcluder", 
        label = "Categories to include/exclude", 
        choices = categ, 
        options = list(
          `actions-box` = TRUE, 
          size = 10,
      ...
      tabItem(tabName = "overview",
              fluidPage(
                ...
                fluidRow(
                  ...
                  box(width = 12, height = 300,
                      title = "Waste movement quantities (in tons)", status = "warning",
              ...
              fluidRow(
                box(width = 12, height = 375,
                    title = "Classification of waste amounts (in tons)", status = "warning",
...
server <- function(input, output, session) {
  ...
  observe({
    if(input$allreg == 0) 
    {
      ...
      updatePrettyCheckboxGroup(session, inputId = "region","Select Region",choices=reg ,selected=input$region)
    ...
    updatePickerInput(session, inputId = "categExcluder", label = "Categories to include/exclude", choices = categ, selected = categ)
  ...
  output$WasteTransAndTreatment <- renderPlotly({
    ...
    transported2 <- transported2[transported2$variable %in% c(input$categExcluder),]
    ...
    print(
      ggplotly(height = 200,
               ...
               ggplot(data = agregTable, aes(x = Category, y = Quantity, fill = Category), binwidth = 0) + 
                 geom_bar(stat = "identity", width = 1) + geom_text(aes(x = Category, 
                                                                                y = Quantity, label = round(Quantity, 4)), 
                                                                    position = position_dodge(width = 0.5),
                                                                    size = 7 - sqrt(13),
                                                                    vjust="inward",hjust="inward") +
                 scale_y_continuous(limits=c(0,max(range(agregTable$Quantity))),expand = c(0.1,0.1)) + 
                 ...
                 scale_fill_manual(values = c("Waste received from other organizations" = "black", 
                                              "Waste transmitted to other organizations" = "orange", 
                                              "Treated and destructed by other organizations" = "blue",
                                              "Waste used by organizations" = "red",
                                              "Transported to landfills by means of organizations" = "green")) +
  ...
  output$Classification <- renderPlotly({
    ...
    # validate(
    #   need(!is.null(agregTable$Region), "Please select year/region to display"
    #   )
    # )
    ...
    print(
      ggplotly(
        ...
        ggplot(agregTable, aes(x = Region, y = Quantity, fill = Class )) + 
          ...
          ylab("Classification of waste amounts (in tons)") + 
          theme(legend.position = "none",
                axis.text.x = element_text(family = "Verdana",
                                           face = "plain"),
                axis.title.y = element_blank(),
...
#To do:
#generate data annual for classes
#4